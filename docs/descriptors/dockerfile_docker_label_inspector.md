...
## docker-label-inspector documentation

- Visit [Official Web Site](https://github.com/garethr/docker-label-inspector#readme){target=_blank}
- See [How to configure docker-label-inspector rules](https://github.com/garethr/docker-label-inspector#linter){target=_blank}
- See [Index of problems detected by docker-label-inspector](https://github.com/garethr/docker-label-inspector#linter){target=_blank}
...
- Enable docker-label-inspector by adding `DOCKERFILE_DLI` in [ENABLE_LINTERS variable](/configuration/#activation-and-deactivation)
- Disable docker-label-inspector by adding `DOCKERFILE_DLI` in [DISABLE_LINTERS variable](/configuration/#activation-and-deactivation)

| Variable | Description | Default value |
| ----------------- | -------------- | -------------- |
| DOCKERFILE_DLI_ARGUMENTS | User custom arguments to add in linter CLI call<br/>Ex: `-s --foo "bar"` |  |
| DOCKERFILE_DLI_FILTER_REGEX_INCLUDE | Custom regex including filter<br/>Ex: `(src|lib)` | Include every file |
| DOCKERFILE_DLI_FILTER_REGEX_EXCLUDE | Custom regex excluding filter<br/>Ex: `(test|examples)` | Exclude no file |
| DOCKERFILE_DLI_FILE_EXTENSIONS | Allowed file extensions. `"*"` matches any extension, `""` matches empty extension. Empty list excludes all files<br/>Ex: `[".py", ""]` | Exclude every file |
| DOCKERFILE_DLI_FILE_NAMES_REGEX | File name regex filters. Regular expression list for filtering files by their base names using regex full match. Empty list includes all files<br/>Ex: `["Dockerfile(-.+)?", "Jenkinsfile"]` | `["Dockerfile"]` |
| DOCKERFILE_DLI_DISABLE_ERRORS | Run linter but disable crash if errors found | `false` |
